 ...
 */
static rstatus_t conf_server_init(struct conf_server *cs) {
...
static rstatus_t conf_pool_init(struct conf_pool *cp, struct string *name) {
  rstatus_t status;
...
static void conf_dump(struct conf *cf) {
  ...
  log_debug(LOG_VVERB, "  listen: %.*s", cp->listen.pname.len,
            cp->listen.pname.data);
  ...
  log_debug(LOG_VVERB, "  hash_tag: \"%.*s\"", cp->hash_tag.len,
            cp->hash_tag.data);
  log_debug(LOG_VVERB, "  client_connections: %d", cp->client_connections);
  ...
  log_debug(LOG_VVERB, "  server_failure_limit: %d", cp->server_failure_limit);

  log_debug(LOG_VVERB, "  datastore: ");
  log_debug(LOG_VVERB, "    %.*s", cp->conf_datastore->name.len,
            cp->conf_datastore->name.data);

  log_debug(LOG_VVERB, "  dyn_seed_provider: \"%.*s\"",
            cp->dyn_seed_provider.len, cp->dyn_seed_provider.data);
  ...
  log_debug(LOG_VVERB, "  dyn_listen: %.*s", cp->dyn_listen.pname.len,
            cp->dyn_listen.pname.data);
  ...
  log_debug(LOG_VVERB, "  read_consistency: \"%.*s\"", cp->read_consistency.len,
            cp->read_consistency.data);

  log_debug(LOG_VVERB, "  write_consistency: \"%.*s\"",
            cp->write_consistency.len, cp->write_consistency.data);

  log_debug(LOG_VVERB, "  stats_interval: %lu", cp->stats_interval);
  log_debug(LOG_VVERB, "  stats_listen: %.*s", cp->stats_listen.pname.len,
            cp->stats_listen.pname.data);

  log_debug(LOG_VVERB, "  enable_gossip: %s",
            cp->enable_gossip ? "true" : "false");
...
static rstatus_t conf_yaml_init(struct conf *cf) {
  int rv;
...
static rstatus_t conf_token_next(struct conf *cf) {
  int rv;
...
static rstatus_t conf_event_next(struct conf *cf) {
  int rv;
...
static rstatus_t conf_push_scalar(struct conf *cf) {
  rstatus_t status;
  struct string *value;
  uint8_t *scalar;
  uint32_t scalar_len;
...
static char *conf_set_string(struct conf *cf, struct command *cmd, void *conf) {
  rstatus_t status;
  uint8_t *p;
...
static char *conf_set_listen(struct conf *cf, struct command *cmd, void *conf) {
  rstatus_t status;
  struct string *value;
...
static char *conf_add_server(struct conf *cf, struct command *cmd, void *conf) {
  rstatus_t status;
  struct string *value;
  ...
  *pfield = (struct conf_server *)dn_zalloc(sizeof(struct conf_server));
  ...
  /* parse "hostname:port:weight [name]" or "/path/unix_socket:weight [name]"
   * from the end */
  p = value->data + value->len - 1;
  start = value->data;
  addr = NULL;
  ...
  if (k != delimlen) {
    return "has an invalid \"hostname:port:weight [name]\"or "
           "\"/path/unix_socket:weight [name]\" format string";
  }
  ...
  if (value->data[0] != '/') {
    field->port = dn_atoi(port, portlen);
    if (field->port < 0 || !dn_valid_port(field->port)) {
      return "has an invalid port in \"hostname:port:weight [name]\" format "
             "string";
    }
  }
...
/*
 * Well, this just blows. Copied from conf_add_server() there is a colon
 * delimited string in the yaml that requires a few levels of magic in order to
 * guess the structure of, rather than doing the right thing and making proper
 * fields. Need to fix however, there's bigger fish to deep fry at this point.
 */
static char *conf_add_dyn_server(struct conf *cf, struct command *cmd,
                                 void *conf) {
  rstatus_t status;
  struct array *a;
  struct string *value;
  ...
  uint32_t k, delimlen, pnamelen, addrlen, portlen, racklen, tokenslen, namelen,
      dclen;
  struct string address;
  char delim[] = " ::::";
  ...
  if (k != delimlen) {
    return "has an invalid format must match \"hostname:port:rack:dc:tokens "
           "[name]\"";
  }
...
static char *conf_set_tokens(struct conf *cf, struct command *cmd, void *conf) {
  uint8_t *p = conf;
...
static char *conf_set_num(struct conf *cf, struct command *cmd, void *conf) {
  uint8_t *p;
  int num, *np;
  struct string *value;
...
static char *conf_set_hash(struct conf *cf, struct command *cmd, void *conf) {
  uint8_t *p;
  hash_type_t *hp;
  struct string *value;
  ...
  *hp = get_hash_type(value);
  if (*hp == HASH_INVALID) return "is not a valid hash";
  return CONF_OK;
}

static char *conf_set_deprecated(struct conf *cf, struct command *cmd,
                                 void *conf) {
...
static char *conf_set_hashtag(struct conf *cf, struct command *cmd,
                              void *conf) {
  rstatus_t status;
  uint8_t *p;
  struct string *field, *value;
...
static struct command conf_commands[] = {
    ...
    {string("distribution"), conf_set_deprecated,
    ...
    {string("data_store"), conf_set_num,
    ...
    {string("server_connections"), conf_set_deprecated,
     offsetof(struct conf_pool, deprecated)},
    ...
    {string("dyn_write_timeout"), conf_set_num,
     offsetof(struct conf_pool, dyn_write_timeout)},

    {string("dyn_listen"), conf_set_listen,
     offsetof(struct conf_pool, dyn_listen)},

    {string("dyn_seed_provider"), conf_set_string,
     offsetof(struct conf_pool, dyn_seed_provider)},

    {string("dyn_seeds"), conf_add_dyn_server,
     offsetof(struct conf_pool, dyn_seeds)},

    {string("dyn_port"), conf_set_num, offsetof(struct conf_pool, dyn_port)},

    {string("dyn_connections"), conf_set_num,
     offsetof(struct conf_pool, dyn_connections)},

    {string("rack"), conf_set_string, offsetof(struct conf_pool, rack)},

    {string("tokens"), conf_set_tokens, offsetof(struct conf_pool, tokens)},

    {string("gos_interval"), conf_set_num,
     offsetof(struct conf_pool, gos_interval)},

    {string("secure_server_option"), conf_set_string,
     offsetof(struct conf_pool, secure_server_option)},

    {string("pem_key_file"), conf_set_string,
     offsetof(struct conf_pool, pem_key_file)},

    {string("recon_key_file"), conf_set_string,
     offsetof(struct conf_pool, recon_key_file)},

    {string("recon_iv_file"), conf_set_string,
    ...
    {string("read_consistency"), conf_set_string,
     offsetof(struct conf_pool, read_consistency)},

    {string("write_consistency"), conf_set_string,
     offsetof(struct conf_pool, write_consistency)},

    {string("stats_listen"), conf_set_listen,
     offsetof(struct conf_pool, stats_listen)},

    {string("stats_interval"), conf_set_num,
     offsetof(struct conf_pool, stats_interval)},

    {string("enable_gossip"), conf_set_bool,
     offsetof(struct conf_pool, enable_gossip)},

    {string("mbuf_size"), conf_set_num, offsetof(struct conf_pool, mbuf_size)},
...
static rstatus_t conf_begin_parse(struct conf *cf) {
  rstatus_t status;
  bool done;
...
static rstatus_t conf_end_parse(struct conf *cf) {
  rstatus_t status;
  bool done;
...
static rstatus_t conf_parse_core(struct conf *cf, void *data) {
  rstatus_t status;
...
static rstatus_t conf_parse(struct conf *cf) {
  rstatus_t status;
...
static rstatus_t conf_validate_document(struct conf *cf) {
  rstatus_t status;
  uint32_t count;
  bool done;
...
static rstatus_t conf_validate_tokens(struct conf *cf) {
  rstatus_t status;
  bool done, error;
  int type;
...
static rstatus_t conf_validate_structure(struct conf *cf) {
  rstatus_t status;
  int type, depth;
    ...
    switch (type) {
      ...
      case YAML_MAPPING_START_EVENT:
        if (depth == CONF_ROOT_DEPTH && count[depth] != 1) {
          error = true;
          log_error(
              "conf: '%s' has more than one \"key:value\" at depth"
              " %d",
              cf->fname, depth);
...
static rstatus_t conf_pre_validate(struct conf *cf) {
  rstatus_t status;
...
static rstatus_t conf_validate_server(struct conf *cf, struct conf_pool *cp) {
...
static rstatus_t conf_validate_pool(struct conf *cf, struct conf_pool *cp) {
  rstatus_t status;
  ...
  if (string_empty(&cp->dyn_seed_provider)) {
    string_copy_c(&cp->dyn_seed_provider,
                  (const uint8_t *)CONF_DEFAULT_SEED_PROVIDER);
  }
  ...
  if (cp->mbuf_size == CONF_UNSET_NUM) {
    log_debug(LOG_INFO, "setting mbuf_size to default value:%d",
              CONF_DEFAULT_MBUF_SIZE);
    /*
    ...
    if (cp->mbuf_size < CONF_DEFAULT_MBUF_MIN_SIZE ||
        cp->mbuf_size > CONF_DEFAULT_MBUF_MAX_SIZE) {
      log_stderr(
          "mbuf_size: mbuf chunk size must be between %zu and"
          " %zu bytes",
          CONF_DEFAULT_MBUF_MIN_SIZE, CONF_DEFAULT_MBUF_MAX_SIZE);
      return DN_ERROR;
    }
  ...
  if (cp->alloc_msgs_max == CONF_UNSET_NUM) {
    log_debug(LOG_INFO, "setting max_msgs to default value:%d",
    ...
    if (cp->alloc_msgs_max < CONF_DEFAULT_MIN_ALLOC_MSGS ||
        cp->alloc_msgs_max > CONF_DEFAULT_MAX_ALLOC_MSGS) {
      log_stderr(
          "max_msgs: max allocated messages buffer must be between %zu and"
          " %zu messages",
          CONF_DEFAULT_MIN_ALLOC_MSGS, CONF_DEFAULT_MAX_ALLOC_MSGS);
      return DN_ERROR;
    }
  }
  ...
  if (string_empty(&cp->read_consistency)) {
    string_copy_c(&cp->read_consistency, (const uint8_t *)CONF_STR_DC_ONE);
    log_debug(LOG_INFO, "setting read_consistency to default value:%s",
              CONF_STR_DC_ONE);
  }

  if (string_empty(&cp->write_consistency)) {
    string_copy_c(&cp->write_consistency, (const uint8_t *)CONF_STR_DC_ONE);
    log_debug(LOG_INFO, "setting write_consistency to default value:%s",
              CONF_STR_DC_ONE);
  }

  if (cp->stats_interval == CONF_UNSET_NUM) {
    log_debug(LOG_INFO, "setting stats_interval to default value:%d",
              CONF_DEFAULT_STATS_INTERVAL_MS);
    cp->stats_interval = CONF_DEFAULT_STATS_INTERVAL_MS;
  }

  if (!cp->stats_listen.valid) {
    log_error(
        "conf: directive \"stats_listen:\" is missing - using defaults %s",
  ...
  if (dn_strcmp(cp->secure_server_option.data, CONF_SECURE_OPTION_NONE) &&
    ...
    log_error(
        "conf: directive \"secure_server_option:\"must be one of 'none' 'rack' "
        "'datacenter' 'all'");
  }
  ...
  if (string_empty(&cp->pem_key_file)) {
    ...
    log_debug(LOG_INFO, "setting pem key file to default value:%s",
              PEM_KEY_FILE);
  }

  if (string_empty(&cp->recon_key_file)) {
    string_copy_c(&cp->recon_key_file, (const uint8_t *)RECON_KEY_FILE);
    log_debug(LOG_INFO, "setting reconciliation key file to default value:%s",
              RECON_KEY_FILE);
  }

  if (string_empty(&cp->recon_iv_file)) {
    string_copy_c(&cp->recon_iv_file, (const uint8_t *)RECON_IV_FILE);
    log_debug(LOG_INFO, "setting reconciliation IV file to default value:%s",
              RECON_IV_FILE);
  }
...
bool is_secure(secure_server_option_t option, struct string *this_dc,
  ...
  // communication secured between nodes if they are in rack with same name
  // across dcs.
...
static rstatus_t conf_post_validate(struct conf *cf) {
  ASSERT(cf->sound && cf->parsed);
  ASSERT(!cf->valid);
