        ...
        if (ch == ' ' || ch == CR) {
          ...
          if (keylen > MEMCACHE_MAX_KEY_LENGTH) {
            log_error("parsed bad req %" PRIu64
                      ...
                      "maximum key length",
                      r->id, r->type, 16, r->token, p - r->token);
            goto error;
          } else if (keylen == 0) {
            log_error("parsed bad req %" PRIu64
                      " of type %d with an "
      ...
      case SW_RSP_STR:
        ...
        if (ch == ' ' || ch == CR) {
          ...
          switch (p - m) {
            ...
            case 10:
              if (str10cmp(m, 'N', 'O', 'T', '_', 'S', 'T', 'O', 'R', 'E',
                           'D')) {
            ...
            case 12:
              if (str12cmp(m, 'C', 'L', 'I', 'E', 'N', 'T', '_', 'E', 'R', 'R',
                           'O', 'R')) {
                r->type = MSG_RSP_MC_CLIENT_ERROR;
                break;
              }

              if (str12cmp(m, 'S', 'E', 'R', 'V', 'E', 'R', '_', 'E', 'R', 'R',
                           'O', 'R')) {
...
bool memcache_failure(struct msg *r) { return false; }

static rstatus_t memcache_append_key(struct msg *r, uint8_t *key,
                                     uint32_t keylen) {
  struct mbuf *mbuf;
  struct keypos *kpos;
...
static rstatus_t memcache_fragment_retrieval(struct msg *r,
   ...
   * whenever we have multiple messages, we copy the tail message into a new
   * mbuf
   */
  ...
  for (i = 0; i < rack->ncontinuum;
       i++) { /* prepend mget header, and forward it */
...
rstatus_t memcache_fragment(struct msg *r, struct server_pool *pool,
  ...
  if (memcache_retrieval(r)) {
    return memcache_fragment_retrieval(r, pool, rack, frag_msgq, 1);
  }
  return DN_OK;
}
...
void memcache_post_coalesce(struct msg *request) {
  ...
  if (request->is_error || request->is_ferror) {
...
void memcache_swallow_msg(struct conn *conn, struct msg *pmsg,
                          struct msg *msg) {}

rstatus_t memcache_add_auth(struct context *ctx, struct conn *c_conn,
                            struct conn *s_conn) {
  NOT_REACHED();
  return DN_OK;
}
...
bool memcache_is_multikey_request(struct msg *r) { return false; }
