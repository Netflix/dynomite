...
dmsg_parse_host_id(uint8_t *start, uint32_t len,
		struct string *region, struct string *rack, struct dyn_token *token)
{
	uint8_t *p, *q;
	uint8_t *region_p, *rack_p, *token_p;
	uint32_t k, delimlen, region_len, rack_len, token_len;
	char delim[] = "$$";
	delimlen = 2;

	/* parse "region$rack$token : don't support vnode for now */
	...
	region_len = rack_len = token_len = 0;
	...
	region_p = start;
	region_len = len - (token_len + rack_len + 2);
...
dmsg_parse(struct dmsg *dmsg)
{
	...
	dmsg_parse_host_id(host_id, host_id_len, &rnode->region, &rnode->rack, &rnode->token);
