...
dnode_peer_add_local(struct server_pool *pool, struct node *self)
{
    ...
    self->port = pool->d_port;

    self->weight = 0;  /* hacking this out of the way for now */
    self->rack = pool->rack;
    self->is_local = true;
    //TODO-jeb might need to copy over tokens, not sure if this is good enough
    self->tokens = pool->tokens;

    self->family = pool->d_family;
    self->addrlen = pool->d_addrlen;
    self->addr = pool->d_addr;
...
dnode_peer_connect(struct context *ctx, struct node *peer, struct conn *conn)
{
    ...
    if (log_loggable(LOG_VERB)) {
        log_debug(LOG_VERB, "dnode_peer_connect dyn: connect to peer '%.*s'", peer->pname.len,
                peer->pname.data);
    }
    ...
    if (conn->sd < 0) {
        log_error("dyn: socket for peer '%.*s' failed: %s", peer->pname.len,
                peer->pname.data, strerror(errno));
        status = DN_ERROR;
        goto error;
    }
    log_debug(LOG_WARN, "dnode: connected to peer '%.*s' on p %d", peer->pname.len,
            peer->pname.data, conn->sd);


    status = dn_set_nonblocking(conn->sd);
    if (status != DN_OK) {
        log_error("dyn: set nonblock on s %d for peer '%.*s' failed: %s",
                conn->sd,  peer->pname.len, peer->pname.data,
                strerror(errno));
        goto error;
    }


    if (peer->pname.data[0] != '/') {
...
dnode_peer_close(struct context *ctx, struct conn *conn)
{
    ...
    log_debug(LOG_WARN, "dyn: dnode_peer_close on peer '%.*s'", peer->pname.len,
            peer->pname.data);
